            ...
            if (response.estado) {
                ...
                $.each(response.objeto, function (i, row) {
                    $("<tr>").append(
                        ...
                        $("<td>").append(
                            $("<button>").addClass("btn btn-sm btn-primary mr-1").text("Editar").data("productotienda", row),
...
$('#tbProductoTienda tbody').on('click', 'button[class="btn btn-sm btn-primary mr-1"]', function () {

    var model = $(this).data("productotienda")

    $("#txtIdProductoTienda").val(model.IdProductoTienda);
    $("#txtPrecioVenta").val(model.PrecioUnidadVenta);
...
$('#btnGuardarCambios').on('click', function () {
    ...
    if (!camposvacios) {
        ...
        var request = {
            oProductoTienda: {
                IdProductoTienda: parseInt($("#txtIdProductoTienda").val()),
                PrecioUnidadVenta: parseFloat($("#txtPrecioVenta").val()),
            }
        }
        
        if (parseInt($("#txtIdProductoTienda").val()) == 0) {
            ...
            AjaxPost("../frmProductoTienda.aspx/Editar", JSON.stringify(request),
                function (response) {
                    $(".modal-body1").LoadingOverlay("hide");
                    if (response.estado) {
