...
$('#btnBuscarCliente').on('click', function () {
    ...
    AjaxPost("../frmCrearVenta.aspx/ObtenerxCliente", JSON.stringify(),
        function (response) {
            $(".modal-body").LoadingOverlay("hide");
            if (response.estado) {
                $.each(response.objeto, function (i, row) {
...
$('#tbCliente tbody').on('click', 'button[class="btn btn-sm btn-primary ml-2"]', function () {
    var model = $(this).data("cliente")
    $("#txtclientenombres").val(model.Nombre);
    $("#txtclientedocumento").val(model.NumeroDocumento);
    $("#txtclientetelefono").val(model.Telefono);
    $("#txtclientedireccion").val(model.Direccion);
    $("#txtclientedescuento").val(model.Descuento);
...
$('#btnAgregar').on('click', function () {
    ...
    if (!existe_codigo) {
        ...
        var importetotal = (parseFloat($("#txtproductoprecio").val()) - parseFloat($("#txtclientedescuento").val())) * parseFloat($("#txtproductocantidad").val());
        $("<tr>").append(
            $("<td>").append(
...
$('#btnTerminarGuardarVenta').on('click', function () {
    ...
    VENTA = "<VENTA>" +
        ...
        "<MetodoPago>" + $("#cbometododepago").val() + "</MetodoPago>" +
        "</VENTA >";
    ...
    DETALLE_CLIENTE = "<DETALLE_CLIENTE><DATOS>" +
        ...
        "<Descuento>" + $("#txtclientedescuento").val() + "</Descuento>" +
...
function calcularDeuda() {
    ...
    var totalcosto = parseFloat($("#txttotal").val().trim());
    var deuda = 0;
    deuda = (totalcosto > montopago ? totalcosto - montopago : 0);

    $("#txtdeuda").val(deuda.toFixed(2));
}
